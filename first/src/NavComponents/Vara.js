...
function Vara() {
  ...
  return (
    <div ref={varaRef} style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      <header style={{ padding: '20px', textAlign: 'center', fontFamily: 'Anta, serif', overflow: 'hidden' }}>
        <h1 style={{ transform: isVisible ? 'translateX(0)' : 'translateX(-100vw)', transition: 'transform 2s', display: 'inline-block' }}>CURIOUS SHE , </h1>
        <h1 style={{ transform: isVisible ? 'translateX(0)' : 'translateX(100vw)', transition: 'transform 2s', display: 'inline-block', marginLeft: '10px' }}>CONFIDENT SHE</h1>
      </header>
